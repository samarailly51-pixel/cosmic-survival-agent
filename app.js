...
function generateMissionReport() {
  ...
  const stability = getBaseStability();
  const diagnosis = getRiskDiagnosis();
  const latestLogs = (state.missionLog || []).slice(-5).reverse();
  const metrics = Object.entries(state.metrics)
    .map(([key, value]) => `- ${metricConfig[key].label}: ${value}${metricConfig[key].unit}`)
    .join("\n");
  const tasks = protocol.tasks
    .map((task) => `- [${completed.has(task.key) ? "x" : " "}] ${task.label}`)
    .join("\n");
  const logs = latestLogs.length
    ? latestLogs
        .map((entry) => {
          const effects = entry.effects?.length ? `\n  - Effects: ${entry.effects.join("; ")}` : "";
          return `- Sol ${String(entry.sol).padStart(3, "0")} / ${entry.title}: ${entry.body}${effects}`;
        })
        .join("\n")
